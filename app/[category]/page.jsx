...
// Category emoji icons — used as product card visual
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 14px 0" }}>
        ...
        <div style={{
          ...
          background: isTop3 ? "#fff0ee" : "#f5f2ee",
          border: isTop3 ? "1.5px solid #f4a995" : "1.5px solid #e8e4df",
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 6, flexWrap: "wrap", marginBottom: 5 }}>
            ...
            {item.tag && (
              <span style={{ fontSize: 12, fontWeight: 700, padding: "2px 7px", borderRadius: 4, background: tag.bg, color: tag.color, whiteSpace: "nowrap", flexShrink: 0 }}>
                {item.tag}
              </span>
            )}
          </div>
    ...
    <div style={{ background: "#fff", border: "1.5px solid #d4d0cb", borderRadius: 12, overflow: "hidden" }}>
      ...
      {open && (
        <div style={{ padding: "0 16px 14px", borderTop: "1px solid #ede9e4" }}>
          <p style={{ fontSize: "clamp(12px, 3vw, 13px)", color: "#3d3d3d", lineHeight: 1.7, marginTop: 12 }}>{faq.answer}</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ background: "#fff", border: "1.5px solid #d4d0cb", borderRadius: 14, padding: "14px" }}>
      <div style={{ display: "flex", gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: 10, background: "#e8e4df", flexShrink: 0 }} />
        <div style={{ flex: 1 }}>
...
export default function CategoryPage({ slug }) {
  ...
  const seo = SEO[slug] || {
    title: `Top 10 ${slug ? slug.replace(/-/g, " ") : ""} Portugal 2026 — ai10pt.top`,
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 40 }}>
            {loading ? (
              ...
              list.items
                ...
                .map((item, i) => <ProductCard key={item.rank} item={item} index={i} categoryEmoji={categoryEmoji} />)
            )}
          </div>
          ...
          {!loading && list?.faqs?.length > 0 && (
            <div style={{ marginBottom: 40 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {list.faqs.map((faq, i) => <FAQItem key={i} faq={faq} index={i} />)}
              </div>
            </div>
          )}
        ...
        <div style={{ background: "#1a1a1a", padding: "28px 16px" }}>
          <div style={{ maxWidth: 820, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 400 }}>
              ...
              <button style={{ fontSize: 14, fontWeight: 700, padding: "13px", borderRadius: 10, background: "#c0392b", color: "#fff", border: "none", cursor: "pointer", minHeight: 48 }}>
                Subscrever grátis
              </button>
            </div>
          </div>
