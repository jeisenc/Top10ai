...
export default function Top10Page() {
  ...
  const [activeCategory, setActiveCategory] = useState("wireless-headphones");

  const todayFormatted = new Date().toLocaleDateString("pt-PT", {
    weekday: "long", year: "numeric", month: "long", day: "numeric"
  });

  // Fetch ALL categories at once on load
  useEffect(() => {
    async function fetchAllLists() {
      ...
      // Build a map of category -> most recent list for that category
      const map = {};
  ...
  const currentList = lists[activeCategory] || null;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cat-btn { font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 500; padding: 7px 16px; border-radius: 999px; border: 1px solid #ddd; background: #fff; color: #555; cursor: pointer; white-space: nowrap; transition: all 0.15s; }
        ...
        <main style={{ maxWidth: 780, margin: "0 auto", padding: "32px 24px" }}>
          <div style={{ marginBottom: 28, animation: "fadeUp 0.4s ease both" }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(26px, 5vw, 36px)", fontWeight: 800, lineHeight: 1.15, color: "#0f0f0f", marginBottom: 10, letterSpacing: "-0.5px" }}>
              {loading ? "A carregar..." : currentList?.category_pt || "Os melhores produtos"}
            </h1>
            {!loading && currentList?.headline && (
              <p style={{ fontSize: 15, color: "#555", lineHeight: 1.6, maxWidth: 580 }}>{currentList.headline}</p>
            )}
